...
function genie_reservation_evenement_cloture_dist($t) {
	...
	$sql = sql_select('id_reservations_detail,t1.id_evenement,date_fin', 'spip_reservations_details t1,spip_evenements t2', 't1.statut="accepte" AND
	...
	while ($data = sql_fetch($sql)) {
		...
		if (strtotime($date_fin) <= strtotime($date)) {
			...
			objet_instituer('reservations_detail', $data['id_reservations_detail'], array(
				'statut' => 'cloture'
			));
	...
	if (count($id_evenement) > 0)
		sql_updateq('spip_evenements', array(
			'action_cloture' => 3
		), 'id_evenement IN (' . implode(',', $id_evenement) . ')');
